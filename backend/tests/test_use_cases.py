...
class TestCreateTaskUseCase:
    ...
    def test_create_task_use_case_success(self):
        ...
        mock_repo.exists.return_value = False  # No duplicate
        mock_repo.create.return_value = InternalTask(
            id=1,
            booking_id=1,
            title="Test task",
        ...
        # Act
        result = use_case.execute(booking_id=1, title="Test task")
    ...
    def test_create_task_use_case_duplicate(self):
        ...
        # Arrange
        mock_repo = Mock(spec=ITaskRepository)
        mock_repo.exists.return_value = True  # Duplicate exists
        
        use_case = CreateTaskUseCase(mock_repo)
        
        # Act & Assert
        with pytest.raises(ValueError, match="already exists"):
            use_case.execute(booking_id=1, title="Duplicate task")
    ...
    def test_create_task_use_case_empty_title(self):
        ...
        with pytest.raises(ValueError, match="Title must not be empty"):
            use_case.execute(booking_id=1, title="")
...
class TestGetTasksUseCase:
    ...
    def test_get_tasks_use_case_success(self):
        ...
        mock_repo.get_by_booking_id.return_value = mock_tasks
    ...
    def test_get_tasks_use_case_empty(self):
        ...
        # Arrange
        mock_repo = Mock(spec=ITaskRepository)
        mock_repo.get_by_booking_id.return_value = []
...
class TestUpdateTaskUseCase:
    ...
    def test_update_task_use_case_success(self):
        ...
        # Arrange
        mock_repo = Mock(spec=ITaskRepository)
        mock_repo.get_by_id.return_value = InternalTask(
            id=1,
            booking_id=1,
            title="Test task",
            status=TaskStatus.OPEN
        )
        mock_repo.update_status.return_value = InternalTask(
            id=1,
            booking_id=1,
            title="Test task",
        ...
        # Act
        result = use_case.execute(task_id=1, status="IN_PROGRESS")
    ...
    def test_update_task_use_case_not_found(self):
        ...
        with pytest.raises(ValueError, match="not found"):
            use_case.execute(task_id=999, status="IN_PROGRESS")
    ...
    def test_update_task_use_case_invalid_status(self):
        ...
        with pytest.raises(ValueError, match="Invalid status"):
            use_case.execute(task_id=1, status="INVALID_STATUS")
